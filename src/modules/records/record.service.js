...
class RecordService {
  ...
  async createRecord({ amount, type, category, record_date, note, created_by }) {
    ...
    const result = recordRepository.insertRecord({ amount: convertMoney.rupeesToPaise(amount), type, category, record_date, note, created_by });
    return { id: result.lastInsertRowid, amount: convertMoney.paiseToRupees(amountInPaise), type, category, record_date, note, created_by: userExists.name };
  }
